...
class SemiParametricPointProcess(nn.Module):
    def __init__(
        self,
        n_types: int,
        max_mean: float,
        ...
        **kwargs,
        ...
        super().__init__()
        self.n_types, self.n_bases = n_types, n_bases
        self.embedding_dim = embedding_dim

        self.embed = nn.Linear(n_types, embedding_dim, bias=False)

        # Loading the prior root alarm probability for each event type.
        self.root_prob = nn.Parameter(torch.FloatTensor(np.array(root_prob)), requires_grad=False)
        self.log_intensities_prior = nn.Parameter(torch.zeros(self.n_types), requires_grad=True)
    ...
    def event_prior_prior_intensities(self, event_seqs):
        # select the prior intensities from self.log_intensities_prior according to the indices from event_seqs
        return self.log_intensities_prior.expand(*(event_seqs.size()[:2]), -1).gather(dim=2, index=event_seqs[:, :, 1:].long()).squeeze(-1)
    ...
    def forward(
        ...
        #     batch_size, T, self.n_types, -1, self.embedding_dim
        # )
        log_basis_feat = self.shallow_net(history_emb).view(
            batch_size, T, self.n_bases + 1, self.embedding_dim
        )
        if target_type >= 0:
            event_embedding = self.embed(F.one_hot(torch.LongTensor([target_type]).to(self.get_model_device()), self.n_types).float())
            log_basis_weights = (log_basis_feat * event_embedding).sum(-1, keepdim=True)
        else:
            event_embedding = self.embed(F.one_hot(torch.arange(self.n_types).to(self.get_model_device()), self.n_types).float()).permute(1, 0).contiguous()
            log_basis_weights = log_basis_feat @ event_embedding
        ...
        **kwargs,
        ...
        for batch in train_dataloader:
            ...
            prior_log_intensities_loss = (
                 -(self.log_intensities_prior * self.root_prob).expand(
                     *(batch.size()[:2]), -1
            ...
            baselines = F.pad(batch[:, :, :1], (0, self.n_types))
            baselines_log_intensities = self.forward(
                baselines,
                onehot=True,
                need_weights=False
            )
            baselines_loss = ((baselines_log_intensities - self.log_intensities_prior.detach())**2).sum()
            loss = nll + prior_log_intensities_loss + baselines_loss
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader):
                ...
                sampled_events_type = -1 * torch.ones(
                    (N * n_samples, self.n_types), dtype=torch.int, device=device
                )
    ...
    def get_seq_contribution(
        ...
        def func(X, p):
            log_intensities = self.forward(
                X, onehot=True, need_weights=False
            )
            # return log_intensities
            log_intensities_specific = torch.gather(
                log_intensities[:, p],
                dim=-1,
                index=torch.argmax(X[:, p, 1:], dim=-1, keepdim=True)
            )
        ...
        log_intensities_events = self.forward(inputs, onehot=True, need_weights=False).gather(dim=2, index=batch[:, :, 1:].long()).squeeze(dim=-1)
        base_log_intensities_events = self.forward(baselines, onehot=True, need_weights=False).gather(dim=2, index=batch[:, :, 1:].long()).squeeze(dim=-1)
